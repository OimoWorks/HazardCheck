...
// 浸水ランクのコード→表示ラベル対応。
//
// ✅ 確認済み: 松山市周辺の実データ（A31a_105、計画規模）で全地物を走査し、
// 出現したコード値は 1〜5 のみ（1: 52,611件, 2: 35,694件, 3: 2,963件,
// 4: 745件, 5: 131件。6は今回のデータ範囲では未出現）だった。これは
// 国土交通省「洪水浸水想定区域図作成マニュアル」に記載の6段階の浸水深
// ランクと一致する。A31a_205（想定最大規模）も同一の6段階コード体系を
// 前提とする（同一属性シリーズ・同一マニュアルに基づくコードのため）。
// 以前ここにあった「十の位で計画規模/想定最大規模を区別する2桁コード」の
// 想定は、実データでは出現しなかったため削除した。
